...
scraping_url = "https://investidor10.com.br"
...
async def get_stock(client: AsyncClient, ticker: str) -> Stock:
    """Get stock information from Investidor10.
    ...
    response = await client.get(
        f"{scraping_url}/acoes/{ticker.lower()}",
        headers=headers,
...
async def list_tickers_most_popular(client: AsyncClient) -> list[str]:
    """List the most popular tickers (About stocks) from Investidor10.
...
async def list_stocks(client: AsyncClient, tickers: list[str]) -> list[Stock]:
    """
    List stocks information from Investidor10.
